...
## 현재 상태 (자동 루프가 세션 재개 시 가장 먼저 읽을 것)

Must/자동화/Should(S1~S10) 전부 처리 완료됨. 남은 항목은 M9(실기기 테스트) 하나뿐이며
🛑 사람 확인 필요 항목이라 자동 진행 불가 (물리 기기 필요).

⚠️ 사람이 아직 안 했을 수도 있는 수동 단계: `docs/supabase-schema.sql` 맨 아래
"추가 마이그레이션 (S7)" 블록(신규 유저 profiles 자동 생성 트리거)을 Supabase
SQL Editor에서 실행했는지 확인 — 안 했으면 실제 로그인 후 저장이 FK 위반으로 실패한다.
이건 자동 루프가 대신 실행할 수 없다 (Supabase 대시보드 접근 필요).

이 상태에서 재개된 세션은:
1. 아래 M9가 사람 쪽에서 해결됐는지 대화 맥락에서 확인
2. 해결된 게 있으면 해당 항목 진행
3. 여전히 다 막혀있으면 새 작업을 지어내지 말고 "대기 중, 변동 없음"으로 짧게 보고하고 종료 — 이 상태에서 토큰을 소모하며 새로운 백로그를 임의로 만들지 않는다
...
- [x] **M8. 참고 가격 문구 노출** — `capture.tsx` 결과 화면에 "AI 추천 참고가이며 실제 거래가와 다를 수 있어요" 같은 문구를 가격 옆에 상시 노출(기획서 리스크 대응). 완료 기준: 화면 코드에 해당 문구 렌더링 확인.
- [ ] **M9. 🛑 사람 확인 필요 — 실기기 테스트** — 여기 도달하면 멈추고 보고만 한다. 물리 기기/EAS 빌드가 필요해 이 환경에서 수행 불가.

## 자동화

- [x] **A1. CI 워크플로우** — `.github/workflows/ci.yml`, mobile/server 각각 typecheck/lint/test를 push마다 자동 실행. server 테스트는 `MOCK_AI=true`로 실행해 실제 API 호출 없음.
...
- [x] **S4. 공유카드 기본 구현** — 사람이 "간단한 기본 디자인으로 바로 구현" 선택. `react-native-view-shot` + `expo-sharing`(스파이크 후보 1)으로 흰 배경 + 제목/설명/가격 + 워터마크 카드를 캡처해 네이티브 공유 시트로 공유. `src/components/ShareCard.tsx`, `src/services/shareCardService.ts`, `capture.tsx`에 "카드 이미지로 공유" 버튼. 완료 기준: typecheck/lint/번들 통과. 색상/로고 등 정식 브랜딩은 추후 `ShareCard.tsx` 스타일만 교체하면 됨.
...
- [x] **S9. 판매완료 피드백 입력**
- [x] **S10. 마이페이지 진입/로그아웃 도선** — `capture` 화면 헤더에 "마이페이지" 버튼 추가, `mypage.tsx`에 "로그아웃" 버튼 추가. S5~S9 구현 후에도 마이페이지로 갈 방법이 화면에 전혀 없던 누락을 발견해 보완. — 기획서상 데이터 플라이휠 핵심(우선순위 최상단). `listing_feedback`에 실제 판매가 저장 + `listings.status`를 `sold`로 갱신. `app/feedback.tsx` (listingId 파라미터) + `mypage.tsx`에서 active 항목에 진입 버튼. 완료 기준: typecheck/lint/번들 통과.

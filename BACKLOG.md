...
- [x] **S9. 판매완료 피드백 입력** — 기획서상 데이터 플라이휠 핵심(우선순위 최상단). `listing_feedback`에 실제 판매가 저장 + `listings.status`를 `sold`로 갱신. `app/feedback.tsx` (listingId 파라미터) + `mypage.tsx`에서 active 항목에 진입 버튼. 완료 기준: typecheck/lint/번들 통과.

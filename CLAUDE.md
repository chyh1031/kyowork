...
- 환경변수: `.env.example` 참고 (`EXPO_PUBLIC_API_BASE_URL`, `EXPO_PUBLIC_SUPABASE_URL`, `EXPO_PUBLIC_SUPABASE_ANON_KEY`)
- `src/lib/supabase.ts` — Supabase 클라이언트 (Auth 세션은 AsyncStorage에 저장). anon key는 RLS로 보호되므로 클라이언트 노출 전제 — service_role 키는 절대 클라이언트에 두지 않음
...
- 명령어: `npx vercel dev` (package.json에 `dev` 스크립트로 넣으면 안 됨 — Vercel이 재귀 호출로 오인해 에러남) / `npm run lint` / `npm run typecheck` / `npm test` (vitest, 실제 API 호출 없음)
